...
Two lattices $L$ and $L'$ are \defnphrase{homothetic} if $L' = tL = \{t\omega : \omega \in L\}$ for some fixed $t \in \R$.
We denote the space of all nontrivial lattices modulo the homothety relation as $\LS = \LS_0 \cup \LS_1$, where $\LS_0$ and $\LS_1$ are the degenerate and non-degenerate lattices, respectively.

Some authors use a slightly different vocabulary to discuss the set of non-degenerate lattices $\LS_1$. Rather than identify all homothetic lattices, we can instead restrict the set of lattices to only those of a certain unit size. More precisely, we restrict to the set of \defnphrase{unimodular lattices}---lattices with determinant equal to $1$, where the determinant of a lattice is the determinant of the matrix whose columns are the generators of the lattice. For our purposes, we will switch between these two interpretations of $\LS_1$ quite freely.
...
We now describe a dynamical flow in the space of non-degenerate lattices.
We can think of a complex lattice $L \in \LS_1$ as sitting inside $\R^2 \cong \C$. Left multiplication of each point in the lattice by
...
\begin{proof}
  A lattice is all integer combinations of vectors in a basis for that lattice; since $X(\Z^2) = Y(\Z^2)$, we know that there exists integer matrices $A$ and $B$ such that $X = YA$ and $Y = XB$.
  So we just need to show that $A$ has determinant 1.
  Note that $X = XBA$, which means that $X(I - BA) = 0$.
  Since $X(\Z^2)$ is nondegenerate, the columns of $X$ are linearly independent, and so $I - BA = 0$ and hence $BA = I$.
  Then $\det(BA) = \det(B) \det(A) = \det(I) = 1$.
  Since $A$ and $B$ are integer, we have that $\det(A) = \det(B) = \pm 1$.
  But also since $X = YA$, we know $1 = \det(X) = \det(Y) \det(A) = 1 \det(A)$, so $\det(A)$ must be 1 and thus $A \in \SLZ$.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &> 2.
  \end{align*}
  Thus $A$ is indeed hyperbolic, and it is straightforward to observe that we indeed have a bijection.
\end{proof}
