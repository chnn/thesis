...
The 3-sphere, the space of unimodular lattices, and 1 to 3 point subsets of the circle---each of these spaces possesses a unique and rich structure.
Remarkably, they are also homeomorphic, providing multiple perspectives of phenomenon occurring in each individual space.

In this document, we examine the modular flow and its periodic orbits in the space of unimodular lattices.
This is a simple dynamical system whose a periodic orbits can be described in terms of hyperbolic elements of the modular group, $\PSLZ$.
...
\chapter{The space of unimodular lattices}
...
The \defnphrase{determinant of a lattice} is the determinant of the matrix whose columns are the generators of the lattice, considered as pairs of real numbers---this quantity is the area of a single cell in the lattice.
If a lattice has determinant 1 or $-1$, we say that the lattice is \defnphrase{unimodular}. 
...
We denote the space of all non-trivial unimodular lattices as $\LS = \LS_0 \cup \LS_1$, where $\LS_0$ and $\LS_1$ are the degenerate and non-degenerate lattices, respectively.
This space can alternatively be defined as the quotient space of all lattices modulo a homothety relation.
Two lattices are homothetic if one is the scalar multiple of the other, i.e. if the generators of one lattice is a multiple of the generators of the other lattice.
...
\begin{equation}\label{eq:delta_t}
  ...
  \end{pmatrix}
\end{equation}
produces another unimodular lattice.
...
\begin{align}\label{eq:iso_pi1_b3}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \begin{tikzpicture}
      \braid[ultra thick, width=5mm, height=7mm] s_1 s_2 s_1;
      \draw node[fill=white] at (2,-1.25) {=};
    \end{tikzpicture}
    \begin{tikzpicture}
      \braid[ultra thick, width=5mm, height=7mm] s_2 s_1 s_2;
    \end{tikzpicture}
...
\end{equation*}
When $k = 4$ or $k = 6$, these series converge and is known as \defnphrase{Eisenstein series}.
...
\end{enumerate}
Thus for any unimodular lattice $L$, we can obtain a unique rescaling $L' = tL$ for some $t > 0$ such that
\begin{equation*}
  |G_4(L')| + |G_6(L')| = 1
\end{equation*}
...
We have observed the periodic orbits of a simple flow in the space of lattices in Section~\ref{subsec:lattice_flow}, and we have just seen that the space of lattices is homeomorphic to the 3-sphere.
...
Thus we can say that in the 3-sphere, \emph{a periodic orbit of the modular flow in the space of lattices determines a link with the trefoil knot}.
...
\begin{proof}
  ...
  Since conjugacy is a transitive relation, this means that $A$ is conjugate to the word in $P$ and $Q$ as well.
\end{proof}
...
\begin{defn}
  ...
  \begin{equation*}
    s(h, k) = \sum_{r=1}^{k-1} \frac r k \left( \! \left( \frac {hr} k \right) \! \right)
  \end{equation*}
  where
  \begin{equation*}
    ((x)) = \begin{cases}
...
In general, we denote the set of all subsets with at most $k$ points of a space X as $\exp_k X$, and refer to it as a \defnphrase{finite subset space}.
...
\end{equation*}
Intuitively, the Hausdorff distance between two subsets is the minimum amount they must be ``inflated'' in order to contain each other.
...
The \defnphrase{Voronoi cell} $V(L)$ of a non-degenerate lattice $L$ is any cell chosen from a Voronoi partition of $L$ (any cell will do since every cell in a Voronoi partition of a lattice is a translation of the same polygon).
Or more directly, $V(L)$ is the set of all complex numbers $z$ such that $|z| \leq |z - \omega|$ for all nonzero $\omega \in L$.
If $L$ is a rectangular lattice then $V(L)$ is a rectangle, otherwise it is a hexagon.
...
Since lines passing through the origin are points in $\mathrm{RP}^1 \cong S^1$, we have associated with each non-degenerate lattice $L$ a subset $f(L) \in \exp_3 S^1$.
...
\begin{figure}[t!]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{The three closest lattice points to any vertex the cell define three lines.}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.45\textwidth}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/subset_homeomorphism_step_4.pdf}
    \caption{When the entire lattice is translated so that the midpoint of the cell is at the origin, these three lines intersection with $S^1$ in three points.}
  \end{subfigure}
...
Mathematics, however, remains largely unaffected by developments in computing.
...
The series for $G_4(L)$ and $G_6(L)$ do converge however, and for non-degenerate lattices we can compute the tuple $(G_4(L),\, G_6(L))$ using Mathematica's \mintinline{Mathematica}|WeierstrassInvariants| function.
