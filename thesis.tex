...
Remarkably, they are also homeomorphic, providing multiple perspectives of phenomenon occurring in each space individually.
...
Two lattices $L$ and $L'$ are \defnphrase{homothetic} if they are rescaled versions of each other, i.e. if $L' = tL = \{t x : x \in L\}$ for some fixed $t \in \R$.
...
\begin{equation}\label{eq:delta_t}
  ...
  \end{pmatrix}
\end{equation}
produces another close by nondegenerate lattice.
...
If we continuously act upon $P$ by $\delta_t$ as $0 \leq t \leq 0.96$, we observe a periodic orbit in the space of lattices.
...
Our construction of the modular flow is an non-standard take on a well-known and classically studied dynamical system called \emph{the geodesic flow on the modular surface}.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/geodesics_in_h.pdf}
...
\end{equation*}
of $\mathbb{H}$ contains exactly one point from each one of these orbits, and thus provides a convenient geometrical region that represents the modular surface (the region $\mathcal{F}$ is a \defnphrase{fundamental domain} for $\mathcal{M}$).
...
A \defnphrase{geodesic path} is the generalization of a straight line in Euclidean space to $n$-dimensional manifold spaces---the quickest way between two points across the surface of a manifold is via a geodesic path, just as the quickest way between two points in Euclidean space is via a straight line.
...
Now, suppose that $M$ is a manifold.
Given a point $p \in M$ and a direction $v$ in the space tangent to $M$ at $p$, the \defnphrase{geodesic flow on a $M$} can be simply described as the flow along the geodesic in $M$ that points in the direction of $v$.
...
For manifolds satisfying some additional conditions (\defnphrase{Riemannian manifolds}), one may define the unit tangent bundle.
...
If we project an orbit of the modular flow  back down into $\mathcal{M}$, we recover an orbit of the geodesic flow on $\mathcal{M}$.
...
The fundamental group describes the group structure of all in $X$ up to homotopy.
...
\begin{defn}
  ...
  \end{align*}
  Then if $f_1 : H \to G_1$ and $f_2 : H \to G_2$ are group homeomorphisms, the \defnphrase{amalgamated free product} of $G_1$ and $G_2$ along $H$ is the group with presentation
  \begin{align*}
...
Note that any word in $P$ and $Q$ with $P$ and $Q$ each occurring at least once is a hyperbolic element in $\PSLZ$. Shortly we will see that writing a hyperbolic element $A$ as a $PQ$ word provides insight into the periodic orbit that $A$ defines. First, however, we need understand the connection between $\PSLZ$ and a classic arithmetic object we call the Rademacher class function. The building block of this function is an infinite series with a rather esoteric summand.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/trefoil_link_pqqpqpppq.png}
...
Previously we saw that the periodic orbits of a dynamic system in $\LS_1$ were in bijection with conjugacy classes of hyperbolic elements in $\SLZ$, and that each conjugacy class determined a modular knot in $S^3 \wo K$.
...
The braid word $\beta_A$ for a modular trefoil link $k_A$ seems mimics the $PQ$ decomposition of $A$ closely.
For example, $A = PQQ$ is $\sigma_2 \sigma_2 \sigma_1^{-1}$ as a braid, while $A  = PQQ$ is $\sigma_2 \sigma_2 \sigma_2 \sigma_1^{-1}$.
...
\end{minted}
As discussed in Section~\ref{sec:lattice_to_s3}, all degenerate lattices in $\LS$ present as a trefoil know when viewed in $S^3$.
